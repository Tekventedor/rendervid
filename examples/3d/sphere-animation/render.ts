...
async function main() {
  console.log('\n🌐 Rendering 3D Sphere Animation Example...\n');
  ...
  const maxDuration = Math.min(6, template.output.duration);
  const scale = Math.floor(template.output.width / 4); // 25% width for 4K source
